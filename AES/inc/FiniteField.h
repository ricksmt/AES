...
namespace AES{
	...
	private:
		FiniteField ffAdd(const FiniteField& x) { return ffAdd(element, x); }
		...
		static FiniteField ffMultiply(const FiniteField& a, const FiniteField& b) {
			unsigned int sum = 0x00, running = a;
			for(int i = 0x01; i & 0xFF; i = i << 1){
				if(i & b) sum += running;
	...
	const FiniteField InvSbox[16][16] = {
